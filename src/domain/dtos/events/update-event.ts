export class UpdateEventDto {
  ...
  public static create( obj: {[key: string]: any}): [string?, UpdateEventDto?]{
    ...
    const { name, description, client, guestQuantity, eventType, eventDate, invitation, createdBy } = obj;

    if ( eventType && !['graduación', 'posada', 'xv', 'boda', 'coffee talks', 'otro'].includes(eventType) ) {
      return ['Invalid eventType', undefined];
    }

    return [ undefined, new UpdateEventDto( name, description, client, guestQuantity, eventType, eventDate, invitation, createdBy ) ];
